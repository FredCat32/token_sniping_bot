  ...
  uFactory.events.PairCreated({}, async (error, event) => {
    ...
    try {
      ...
      const gas = await uRouter.methods
        .swapExactTokensForTokens(amountIn, amountOut, path, sniper, deadline)
        .estimateGas({ from: sniper });
      await uRouter.methods
        .swapExactTokensForTokens(amountIn, amountOut, path, sniper, deadline)
        .send({ from: sniper, gas: gas });
      ...
      console.log(
        `Successfully swapped ${AMOUNT} WETH for ${web3.utils.fromWei(
